...
export class StatusApiController 
{
    ...
    async createOrUpdateStatusFile(@Request() req, @UploadedFiles() files , @Headers() headers): Promise<ResponseMessage>
    {
        ...
        const statusDir=this.configurationService.get('status_dir');
        ...
        if( files && files.avatarPdf)
        {
            ...
            const pdfFilename=statusDir+'/status.pdf';
            let err1=null;
    ...
    {
        const statusDir=this.configurationService.get('status_dir');
        const documentFilename=statusDir+'/status.pdf';
        ...
        if( ! fileExists )
        {
            logger.warn('Document status.pdf NOT found !');
            throw new BadRequestException('status.pdf document not found!');
            //return null;
        }
        ...
        res.sendFile(documentFullPathName, (err) => {
            if(err) {
                ...
                throw new BadRequestException('Error while loading document status.pdf !');
            } 
        });
    }
}
